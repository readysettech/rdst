...
export function useTarget() {
  ...
  useEffect(() => {
    ...
    window.addEventListener(TARGET_CHANGED_EVENT, handleTargetChanged as EventListener);

    return () => {
      window.removeEventListener('storage', handleStorage);
      window.removeEventListener(TARGET_CHANGED_EVENT, handleTargetChanged as EventListener);
    };
  }, []);

  const setTarget = useCallback((newTarget: string | null) => {
    const normalizedTarget = newTarget && newTarget.trim() ? newTarget : null;
