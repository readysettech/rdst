/**
 * Scope toolbar for Slow queries (region B of the redesign).
 *
...
export interface SlowQueryControlsProps {
  mode: TopMode
  ...
  onStop: () => void
  hasTarget: boolean
  /** Run status occupies the first row opposite the primary action. */
  status?: ReactNode
  /** Result count occupies the scope row, matching the query library. */
  queryCount?: number
  /** Page hides the toolbar's primary action at idle so the empty-state CTA is the only one. */
  primaryActionHidden?: boolean
  /** Hide the legacy mode switch when this control is embedded as a dedicated
   * live-capture action inside Query Library. */
...
const modeSegments: Array<SegmentedControlSegment<TopMode>> = [
...
export function SlowQueryControls({
  mode,
  setMode,
  source,
  setSource,
  ...
  // The primary action uses the screen's user-facing language in every state.
  const primaryLabel =
  ...
  return (
    <m.div
      ...
      className="space-y-3"
    >
      {/* Run state and the primary action share the first row. */}
      <HStack
        className="justify-between items-center gap-3 flex-wrap"
        data-testid="slow-query-status-row"
      >
        <div>{status}</div>
      ...
      {/* Scope controls and result count share the second row. */}
      <HStack
        className="justify-between items-center gap-3 flex-wrap"
        data-testid="slow-query-scope-row"
      >
        <HStack className="gap-3 items-center flex-wrap">
          {/* Historical / Realtime mode filter — parameterises the run, so it's
              a radio segmented control (selection follows focus), disabled while
              a run is in flight exactly as before. */}
          <Show when={showModeSwitcher}>
            <SegmentedControl
          ...
          {/* ui-new Disclosure couples trigger and panel into one bordered
              container. This toolbar needs an inline design-system trigger with
              the panel spanning the full row below it. */}
          <Button
            size="small"
            variant="primary"
            modifier="link"
            ...
            className="gap-1.5 px-3 h-8 rounded-lg text-content-layout-2 no-underline hover:text-content-layout-1 hover:bg-surface-layout-2"
        ...
        <Show when={queryCount !== undefined}>
            ...
            <div className="rounded-xl bg-surface-layout-2/40 p-4 shadow-elevation-1">
              <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                <Show when={mode === 'historical'}>
                  <FilterField id="top-source" label="Query source">
                    <BaseInputSelect
                      id="top-source"
                      name="source"
                  ...
                  <FilterField id="top-min-freq" label="Minimum executions">
                    <BaseInputText
                      id="top-min-freq"
                      name="min-freq"
                      type="number"
                  ...
                  <FilterField id="top-min-load" label="Minimum load (%)">
                    <BaseInputText
                      id="top-min-load"
                      name="min-load"
                      type="number"
                ...
                <FilterField id="top-limit" label="Results">
                  <BaseInputSelect
                    id="top-limit"
                    name="limit"
                ...
                <Show when={mode === 'historical'}>
                  <div className="col-span-2 md:col-span-3">
                    <FilterField id="top-filter" label="SQL pattern (regex)">
                      <BaseInputText
                        id="top-filter"
                        name="filter"
                        value={filterPattern}
                        ...
                        aria-describedby={
                          filterPatternError ? 'top-filter-error' : undefined
                        }
                      />
                      <Show when={!!filterPatternError}>
                        <div id="top-filter-error">
                          <Text
                            level="caption"
                            className="text-content-negative-plain"
                          >
