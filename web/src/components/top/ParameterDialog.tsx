/**
 * Parameter-value entry for a parameterized query, as a blocking modal or as
 * an inline panel (`presentation`).
 */
...
interface ParameterDialogProps {
  ...
  /**
   * `'modal'` blocks the page it belongs to. `'inline'` renders the same form
   * as a panel in the surrounding flow, for surfaces that are already an
   * overlay — a dialog stacked on the analyze drawer would cover it and, as a
   * higher dismissable layer, leave everything under it unclickable.
   */
...
export function ParameterDialog({
  ...
  const footer = (
    <div className="flex items-center justify-end gap-3">
      {residualError ? (
        <Text level="caption" className="mr-auto text-content-negative-soft">
          {residualError}
        </Text>
      ) : null}
      <Button
  ...
  // Inline the form flows with the surrounding scroll container, so it takes
  // its height from its content instead of claiming a share of the viewport.
  const body = (
    <div
      className={
        isInline
    ...
    >
      ...
      <div className="flex flex-col min-h-0">
        ...
        <div
          className={`bg-surface-layout-1 rounded-lg p-3 border border-border-layout-1 overflow-auto [&_.cm-scroller]:!overflow-visible ${
            isInline ? 'max-h-48' : 'min-h-0 flex-1'
          }`}
        >
          <SQLDisplay
            sql={displayQuery}
      ...
      <div className="flex flex-col min-h-0">
        ...
        <ScrollArea.Root
          className={
            isInline ? 'overflow-hidden' : 'min-h-0 flex-1 overflow-hidden'
          }
        >
          <ScrollArea.Viewport
            className={`w-full custom-scrollbar ${isInline ? '' : 'h-full'}`}
          >
            <div className="space-y-3">
              ...
              <Text level="body-small" className="text-content-layout-3">
                Text values are quoted automatically. Numbers, NULL, TRUE, and
                FALSE are used as entered.
              </Text>
              {sampledByKey.size > 0 || capturedSample ? (
                <Text level="body-small" className="text-content-layout-3">
                  ...
                  (observed values, a captured run) represent production best;
                  the other suggestions are real rows from the table and may
                  produce a very different plan.
                </Text>
              ) : null}
  ...
  if (isInline) {
    ...
    return (
      ...
      >
        ...
        {body}
        <div className="border-t-(length:--border-base) border-border-layout-1 px-4 py-3">
          {footer}
        </div>
      </section>
    )
