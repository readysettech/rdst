...
class SessionInspector:
    """Inspect and summarize ask sessions."""
    ...
    def inspect(self, verbose: bool = False) -> None:
        ...
        if columns:
            names = ", ".join(columns[:10]) + ("..." if len(columns) > 10 else "")
            print(F.label("Column Names", names))
        ...
        if current_state == "error":
            print(F.error(f"Error: {data.get('last_error', 'Unknown error')}"))
