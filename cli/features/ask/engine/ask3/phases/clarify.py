...
def clarify_question(
    ...
    schema_insufficient_ambs = [
        a for a in report.ambiguities if a.category == "schema_insufficient"
    ]
    if schema_insufficient_ambs:
        logger.info(
            "Schema insufficiency detected in clarify phase, deferring to generate phase for expansion"
        )
        # Store hints for expansion phase
        for amb in schema_insufficient_ambs:
            if hasattr(amb, "missing_concepts"):
                ctx.clarifications["missing_concepts"] = ",".join(
                    getattr(amb, "missing_concepts", [])
                )
            if hasattr(amb, "requested_tables"):
                ctx.clarifications["requested_tables"] = ",".join(
                    getattr(amb, "requested_tables", [])
                )
        presenter.info(
            "Schema may be incomplete for this query, will attempt expansion..."
        )
        return ctx
    ...
    # In non-interactive mode, only apply options that pass the ranked gate.
    if ctx.no_interactive:
        resolutions = [
            resolve_ranked_ambiguity(ambiguity) for ambiguity in report.ambiguities
        ]
        apply_selections = bool(resolutions) and all(
            resolution.action == "select" for resolution in resolutions
        )
        answers = {}
        for ambiguity, resolution in zip(report.ambiguities, resolutions):
            resolution_data = resolution.to_dict()
            resolution_data["applied"] = apply_selections
            ctx.clarification_resolutions.append(resolution_data)
            if apply_selections:
                answers[ambiguity.category] = resolution.selected_text or ""
                presenter.clarification_selected(resolution.selected_text or "")
        ctx.clarifications.update(answers)
        if answers:
            ctx.refined_question = _build_refined_question(ctx.question, answers)
        else:
            ctx.mark_error(
                "The question requires clarification and no interpretation passed "
                "the automatic confidence gate"
            )
        return ctx
...
def _build_interpretations(ambiguities) -> list[Interpretation]:
    ...
    for i, amb in enumerate(ambiguities, 1):
        ...
        for j, interp_opt in enumerate(amb.possible_interpretations):
            description = interp_opt.text
            likelihood = interp_opt.score
