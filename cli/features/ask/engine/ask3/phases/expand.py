...
def expand_schema(
    ...
    if new_tables:
        selected_names = {table.casefold() for table in current_tables | new_tables}
        ctx.filtered_tables = [
            table
            for table in ctx.all_available_tables
            if table.casefold() in selected_names
        ]

        # Re-filter the original rich schema. Rebuilding from SchemaInfo loses
        # enum profiles, business context, and relationship details.
        from .filter import _filter_schema_text

        ctx.schema_formatted = _filter_schema_text(
            ctx.schema_full_formatted or ctx.schema_formatted, ctx.filtered_tables
        )

        ctx.increment_expansion()

        presenter.schema_expanded(
            added=list(new_tables), total=len(ctx.filtered_tables)
        )
