...
class DemoCommand:
    ...
    def _load(self, **_kwargs) -> RdstResult:
        ...
        try:
            import logging
            stackoverflow = import_module("features.demo.load_stackoverflow")
            logging.basicConfig(
                level=logging.INFO,
            ...
            engine, params = stackoverflow.parse_database_url(db_url)
            from pathlib import Path
            download_dir = Path.home() / ".rdst" / "cache" / "stackexchange"
            download_dir.mkdir(parents=True, exist_ok=True)

            split = stackoverflow.is_split_site(DEMO_SITE, download_dir)
            if split:
                archive_map = {}
                for table in stackoverflow.DEFAULT_TABLES:
                    path = stackoverflow.download_table_archive(
                        DEMO_SITE, stackoverflow.TABLES[table], download_dir
                    )
                    if path:
                        archive_map[table] = path
            else:
                combined = stackoverflow.download_combined_archive(DEMO_SITE, download_dir)
                if not combined:
                    return RdstResult(False, "Failed to download archive.")
                archive_map = {t: combined for t in stackoverflow.DEFAULT_TABLES}
        ...
        try:
            conn = stackoverflow.create_connection(engine, params)
            cur = conn.cursor()
            for table in stackoverflow.DEFAULT_TABLES:
                if table not in archive_map:
                    continue
                cur.execute(f"DROP TABLE IF EXISTS {table}")
                cur.execute(stackoverflow.PG_DDL[table])
            cur.close()

            for table in stackoverflow.DEFAULT_TABLES:
                if table not in archive_map:
                    continue
                table_def = stackoverflow.TABLES[table]
                row_count = stackoverflow.load_pg(
