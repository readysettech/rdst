...
def validate_prompt_template(template: str, required_placeholders: list[str]) -> bool:
    """
    Validate that a prompt template contains all required placeholders.

    Args:
        template: The prompt template string
        required_placeholders: List of placeholder names without braces

    Returns:
        True if all placeholders are present, False otherwise
    """
    return all(f"{{{placeholder}}}" in template for placeholder in required_placeholders)


def get_prompt_placeholders(template: str) -> list[str]:
    """
