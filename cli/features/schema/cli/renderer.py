...
from features.schema.events import SchemaCompleteEvent, SchemaErrorEvent, SchemaEvent, SchemaStatusEvent
from features.schema.models import SchemaDetails
...
class SchemaRenderer:
    ...
    def render(self, event: SchemaEvent) -> None:
        ...
        if event.operation == "init" and event.init_result and event.init_result.success:
            summary = {
    ...
    def _render_schema_details(self, details: SchemaDetails) -> None:
        ...
        if not self._single_table_show:
                ...
                for custom_type in details.custom_types:
                    if custom_type.type_category == "enum" and custom_type.enum_values:
                        ...
                        if len(custom_type.enum_values) > 5:
                            values_preview += f"... ({len(custom_type.enum_values)} total)"
                        types_tree.add(
                            f"[{StyleTokens.ACCENT}]{custom_type.name}[/{StyleTokens.ACCENT}] [{StyleTokens.MUTED}](enum)[/{StyleTokens.MUTED}]: [{values_preview}]"
                        )
                    elif custom_type.type_category == "domain" and custom_type.base_type:
                        types_tree.add(
            ...
            if details.metrics:
                ...
                metrics_tree = SimpleTree(f"[bold]Metrics ({len(details.metrics)})[/bold]")
