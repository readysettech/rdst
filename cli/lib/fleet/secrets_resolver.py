"""Secrets Manager password resolver with TTL caching."""
...
# In-memory cache: {arn: (value, expiry_timestamp)}
...
def resolve_secret(
    ...
    """Resolve a password from AWS Secrets Manager.

    Args:
        secret_arn: Full ARN or secret name
        secret_key: JSON key within the secret (default: "password")
        region: AWS region (auto-detected from ARN if not provided)
        ttl_seconds: Cache TTL in seconds

    Returns:
        The resolved secret value, or None on failure.
    """
    # Check cache first
    ...
    try:
        from lib.fleet.aws_auth import get_secretsmanager_client
        ...
        try:
            ...
            if value is None:
                ...
                for alt_key in ["password", "Password", "pass", "db_password", "master_password"]:
        ...
        if "AccessDeniedException" in error_msg or "not authorized" in error_msg.lower():
            logger.debug(
                "Cannot access secret %s: check IAM permissions",
                secret_arn,
            )
...
def _extract_region_from_arn(arn: str) -> Optional[str]:
    """Extract region from an ARN like arn:aws:secretsmanager:us-east-1:123:secret:name."""
    m = re.match(r"arn:aws:secretsmanager:([a-z0-9-]+):", arn)
