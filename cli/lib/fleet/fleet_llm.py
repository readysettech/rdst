...
def build_single_target_insights_prompt(result: Dict[str, Any]) -> str:
    ...
    # Include top queries so the LLM can see actual query patterns
    top_queries = result.get("top_queries") or []
    if top_queries:
        details += f"\n== TOP QUERIES (cumulative since last stats reset, from pg_stat_statements / performance_schema, {len(top_queries)} tracked) ==\n"
        details += "NOTE: Call counts are cumulative since the last pg_stat_statements_reset() or server restart, not per-second rates.\n"
        for i, q in enumerate(top_queries[:15]):
            sql = (q.get("normalized_query") or "")[:300]
            details += (
...
def build_fleet_insights_prompt(results: List[Dict[str, Any]]) -> str:
    ...
    for r in results:
        if r.get("error"):
            lines.append(f"- {r.get('target_name', '?')}: ERROR - {r['error']}")
            continue
        ...
        tracked = metrics.get('tracked_query_count', 0)
        entry = (
            f"- {r.get('target_name', '?')} ({r.get('engine', '?')}, "
            f"{r.get('instance_class') or 'unknown class'}):\n"
        ...
        if tracked == 0:
            ext = "pg_stat_statements" if r.get("engine") == "postgresql" else "performance_schema"
            entry += f"\n  WARNING: {ext} not enabled — no query visibility"

        # Include top queries (cumulative from pg_stat_statements / performance_schema)
        top_queries = r.get("top_queries") or []
        if top_queries:
            entry += f"\n  Top Queries (cumulative since last stats reset, {len(top_queries)} tracked):"
            for q in top_queries[:5]:
                sql = (q.get("normalized_query") or "")[:80]
                entry += f"\n    - [{q.get('query_hash','')[:8]}] calls={q.get('calls',0)}, avg={q.get('avg_time_ms',0):.1f}ms: {sql}"

        # Add per-target workload analysis if available (from --duration captures)
        workload = r.get("workload") or {}
        analysis = workload.get("analysis") or {}
        if analysis:
            rec = analysis.get("readyset_recommendation") or {}
            entry += (
                f"\n  --- Workload Analysis ---"
                f"\n  Readyset Verdict: {rec.get('verdict', '?')} — {rec.get('summary', '')}"
                f"\n  Estimated Cacheable: {rec.get('estimated_cacheable_pct', '?')}%"
            ...
            if candidates:
                ...
                for c in candidates:
                    entry += f"\n    - [{c.get('query_hash', '')[:8]}] calls={c.get('calls', 0)}: {c.get('reason', '')}"
            classifications = analysis.get("query_classifications", [])
            oltp = sum(1 for c in classifications if c.get("type") == "oltp")
            analytical = sum(1 for c in classifications if c.get("type") == "analytical")
            if classifications:
                entry += f"\n  Query Mix: {oltp} OLTP, {analytical} analytical, {len(classifications) - oltp - analytical} other"
