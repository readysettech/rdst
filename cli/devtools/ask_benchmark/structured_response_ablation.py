...
from .pipeline import extract_sql

STRUCTURED_RESPONSE_ABLATION_SCHEMA_VERSION = 1
STRUCTURED_RESPONSE_CELLS = ("plain-sql-response", "structured-json-response")
RDST_ROOT = Path(__file__).resolve().parents[2]


def run_structured_response_ablation(
    *,
    source_run_dir: Path,
    output_dir: Path,
    ...
    """Compare plain SQL with structured JSON under common SQL requirements."""
    ...
    manifest = {
        "artifact_schema_version": STRUCTURED_RESPONSE_ABLATION_SCHEMA_VERSION,
        "run_id": output_dir.name,
        "created_at": datetime.now(timezone.utc).isoformat(),
        "purpose": "internal-structured-response-ablation",
        ...
        "cells": list(STRUCTURED_RESPONSE_CELLS),
        "question_policy": (
        ...
        "generation_policy": {
            ...
            "common_sql_requirements": (
                "one read-only statement, exact projection, no unsupported filters, "
                "schema identifiers only"
            ),
            "plain_response": "SQL text only; no tool or JSON response schema",
            "structured_response": (
                "production seven-field strict JSON/tool response schema"
            ),
            "temperature": 0.0,
            "max_tokens": 800,
    ...
    if failed_gold:
        raise RuntimeError(f"Response ablation gold execution failed: {failed_gold}")
    ...
    with store.run_lock():
        for case_index, case in enumerate(cases):
            ...
            cell_order = (
                ...
                if case_index % 2 == 0
                else tuple(reversed(STRUCTURED_RESPONSE_CELLS))
            )
    ...
    if _implementation_hashes() != implementation_hashes:
        raise RuntimeError("Response ablation implementation changed during execution")
    ...
    try:
        if cell == "plain-sql-response":
            ...
            response = adapter.query(
                ...
                max_tokens=800,
                temperature=0.0,
                purpose="ablation_plain_sql_response",
            )
            sql = extract_sql(response["text"])
        elif cell == "structured-json-response":
            response = generate_sql_from_nl(
                ...
                target_database=case.db_id,
                llm_manager=adapter,
                purpose="ablation_structured_json_response",
                system_message=SQL_GENERATION_SYSTEM_PROMPT,
            )
            ...
            raise ValueError(f"Unknown structured-response ablation cell: {cell}")
...
def _summarize(manifest: dict, attempts: list[dict]) -> dict:
    ...
    baseline = by_cell["structured-json-response"]
    treatment = by_cell["plain-sql-response"]
    ...
    return {
        ...
        "cells": cells,
        "paired_comparison": {
            "baseline": "structured-json-response",
            "treatment": "plain-sql-response",
...
def _render_summary(summary: dict) -> str:
    lines = [
        "# Structured Response-Contract Ablation",
        "",
    ...
    lines.extend(
        [
            "",
            f"Paired plain-response delta: {paired['accuracy_delta']:+.1%}",
            "",
            f"Structured-only correct: {paired['baseline_only_correct_ids']}",
            f"Plain-only correct: {paired['treatment_only_correct_ids']}",
            "",
        ]
    )
    return "\n".join(lines)
...
def _write_completion_receipt(
    ...
    if path.exists() and _load_json(path) != receipt:
        raise ValueError("Structured response ablation completion receipt changed")
    if not path.exists():
