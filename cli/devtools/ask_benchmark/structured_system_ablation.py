...
from .models import BenchmarkCase, ModelSpec, QueryResult
from .oracle import score_results

STRUCTURED_SYSTEM_ABLATION_SCHEMA_VERSION = 1
LEGACY_GENERIC_SYSTEM_PROMPT = "You are a helpful assistant."
STRUCTURED_SYSTEM_CELLS = ("generic-system", "expert-sql-system")
RDST_ROOT = Path(__file__).resolve().parents[2]


def run_structured_system_ablation(
    *,
    source_run_dir: Path,
    output_dir: Path,
    ...
    """Compare only the structured generator's system message."""
    ...
    manifest = {
        "artifact_schema_version": STRUCTURED_SYSTEM_ABLATION_SCHEMA_VERSION,
        "run_id": output_dir.name,
        "created_at": datetime.now(timezone.utc).isoformat(),
        "purpose": "internal-structured-system-ablation",
        ...
        "cells": list(STRUCTURED_SYSTEM_CELLS),
        "question_policy": (
        ...
        "generation_policy": {
            "prompt": "identical production structured generation prompt",
            "response_schema": "identical production SQL generation JSON schema",
            "generic_system_message": LEGACY_GENERIC_SYSTEM_PROMPT,
            "expert_system_message": SQL_GENERATION_SYSTEM_PROMPT,
            "temperature": 0.0,
            "max_tokens": 800,
    ...
    if failed_gold:
        raise RuntimeError(f"System ablation gold execution failed: {failed_gold}")
    ...
    with store.run_lock():
        for case_index, case in enumerate(cases):
            ...
            cell_order = (
                ...
                if case_index % 2 == 0
                else tuple(reversed(STRUCTURED_SYSTEM_CELLS))
            )
                ...
                store.append_attempt(
                    {
                        ...
                        "generation_contract": "structured",
    ...
    if _implementation_hashes() != implementation_hashes:
        raise RuntimeError("System ablation implementation changed during execution")
    ...
    *,
    ...
    try:
        response = generate_sql_from_nl(
            ...
            llm_manager=adapter,
            purpose=f"ablation_structured_{cell.replace('-', '_')}",
            system_message=_system_message(cell),
        )
...
def _system_message(cell: str) -> str:
    if cell == "generic-system":
        return LEGACY_GENERIC_SYSTEM_PROMPT
    if cell == "expert-sql-system":
        return SQL_GENERATION_SYSTEM_PROMPT
    raise ValueError(f"Unknown structured-system ablation cell: {cell}")


def _summarize(manifest: dict, attempts: list[dict]) -> dict:
    cells = []
    by_cell: dict[str, dict[int, bool]] = {}
    for cell in STRUCTURED_SYSTEM_CELLS:
    ...
    baseline = by_cell["generic-system"]
    treatment = by_cell["expert-sql-system"]
...
def _render_summary(summary: dict) -> str:
    lines = [
        "# Structured System-Message Ablation",
        "",
    ...
    lines.extend(
        [
            "",
            f"Paired expert-system delta: {paired['accuracy_delta']:+.1%}",
            "",
            f"Generic-only correct: {paired['baseline_only_correct_ids']}",
            f"Expert-only correct: {paired['treatment_only_correct_ids']}",
            "",
        ]
    )
    return "\n".join(lines)
...
def _write_completion_receipt(
    ...
    if path.exists() and _load_json(path) != receipt:
        raise ValueError("Structured system ablation completion receipt changed")
    if not path.exists():
