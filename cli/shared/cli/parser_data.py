...
and Readyset caching opportunities.""",
        args=[
            ...
            ArgDef(
                ...
                action="store_true",
                dest="readyset_cache",
                help="Explicitly verify the query with a temporary Readyset cache",
            ),
        ],
        examples=[
            (
...
Queries captured by 'rdst top' are automatically saved here as they're detected.""",
        ...
        subcommand_defs=[
            ...
            SubcommandDef(
                name="cache-compare",
                help="Run a temporary Readyset comparison",
                args=[
                    ArgDef(
                        "queries",
                        nargs="*",
                    ...
                    ArgDef(
                        "--target",
                        short="-t",
                        help="Source database target",
                    ),
                    ArgDef(
                        "--interval",
                        type=int,
                        metavar="MS",
                        help="Start executions every N milliseconds per side",
                    ),
                    ArgDef(
                        "--concurrency",
                        short="-c",
                        type=int,
                        metavar="N",
                        help="Maintain N concurrent executions per side",
                    ),
                    ArgDef(
                        "--duration",
                        type=int,
                        metavar="SECS",
                        help="Stop each side after N seconds",
                    ),
                    ArgDef(
                        "--count",
                        type=int,
                        default=100,
                        metavar="N",
                        help="Maximum measured executions for each side (default: 100)",
                    ),
                    ArgDef(
                        "--quiet",
                        action="store_true",
        ...
        subcommands=[
            ...
            ("cache-compare", "Run a temporary Readyset comparison"),
        ],
        examples=[
            ...
            ("rdst query cache-compare my-query --count 100", "Measure source vs temporary Readyset cache"),
        ],
    ),
    "schema": CommandDef(
        name="schema",
        ...
        subcommand_defs=[
            ...
            SubcommandDef(
                ...
                args=[
                    ...
                    ArgDef(
                        "--default-database",
                        help="Override the database name for discovered targets. Use this when RDS reports a missing or wrong DBName (e.g. it falls back to the cluster ID, which isn't a valid DB name).",
                    ),
            ...
            SubcommandDef(
                ...
                args=[
                    ...
                    ArgDef("--yes", short="-y", action="store_true", dest="auto_yes", help="Auto-accept temporary Readyset comparisons (no prompt)"),
                ],
            ),
            SubcommandDef(
                name="diff",
...
def get_main_examples() -> List[Tuple[str, str]]:
    return [
        ...
        ('rdst query cache-compare my-query --target mydb', "Run a temporary Readyset comparison"),
